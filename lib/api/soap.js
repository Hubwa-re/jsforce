 ...
 *
 * @param {Array.<Object>} ids - Id of EmailMessage records to send
 * @returns {Promise.<SoapApi~SendEmailResult>}
 */
SoapApi.prototype.sendEmailMessage = function(ids, callback) {
  var args = {
    '@xmlns': 'urn:partner.soap.sforce.com',
    '@xmlns:ns1': 'sobject.partner.soap.sforce.com',
    'ns1:ids': ids,
...
/**
 * @typedef SoapApi~RenderEmailTemplateResult
 * @prop {Array.<SoapApi~RenderEmailTemplateBodyResult>} bodyResults
 * @prop {Array.<Object>} errors - Errors
 * @prop {Boolean} success - True if successfully converted
 */
/**
 * Replaces merge fields in text bodies of email templates with values from Salesforce records, even for polymorphic fields.
 * The email template bodies and their corresponding whoId and whatId values are specified in the argument.
 *
 * @param {Array.<String>} templateBodies
 * @param {String} whatId References a non-human object, such as an account, an opportunity, a campaign, a case, or a custom object. The whatId is polymorphic, which means that it’s an ID that can refer to more than one type of object, such as a case or an opportunity.
 * @param {String} whoId References a human object, such as a lead, contact, or user. The whoId is polymorphic, which means that it’s an ID that can refer to more than one type of object.
 * @param {Callback.<SoapApi~RenderEmailTemplateResult>} [callback] - Callback function
 * @returns {Promise.<SoapApi~RenderEmailTemplateResult>}
*/
SoapApi.prototype.renderEmailTemplate = function(templateBodies, whatId, whoId, callback) {
  const renderRequests = [{templateBodies, whatId}];
  if (whoId) {
    Object.assign(renderRequests[0], {whoId});
  }
  ...
  var args = {
    '@xmlns': 'urn:partner.soap.sforce.com',
    '@xmlns:ns1': 'sobject.partner.soap.sforce.com',
    'ns1:renderRequests': renderRequests,
  };
  return this._invoke(
    "renderEmailTemplate",
    args,
...
Schemas.RenderEmailTemplateResult = {
  success: 'boolean',
  bodyResults: [],
};
